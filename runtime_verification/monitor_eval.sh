...
if [ -z "$PID" ]; then
    ...
    if [ -f "semcluster_similarity_matrix_FULL.csv" ]; then
        ...
        ls -lh semcluster_similarity_matrix_FULL.csv
        echo ""
        echo "Unique queries in output:"
        python3 -c "import pandas as pd; df = pd.read_csv('semcluster_similarity_matrix_FULL.csv'); print(f'  {df[\"query\"].nunique():,} queries')"
    fi
    exit 0
fi
