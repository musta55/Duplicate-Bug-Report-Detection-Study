#!/bin/bash
# Run FILTERED dataset evaluation in tmux
# This will process 30 queries with ~92 bug reports

SESSION_NAME="semcluster_filtered"
...
tmux send-keys -t $SESSION_NAME "echo 'FILTERED EVALUATION STARTED'" C-m
tmux send-keys -t $SESSION_NAME "echo 'Start time: \$(date)'" C-m
tmux send-keys -t $SESSION_NAME "echo '========================================'" C-m
tmux send-keys -t $SESSION_NAME "" C-m

# Run evaluation with single GPU to avoid memory contention
tmux send-keys -t $SESSION_NAME "CUDA_VISIBLE_DEVICES=0 python3 run_parquet_evaluation.py 2>&1 | tee logs/evaluation_filtered_\$(date +%Y%m%d_%H%M%S).log" C-m
