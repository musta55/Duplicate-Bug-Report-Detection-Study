...
def dtw_distance(ts_a, ts_b, min_dis, max_dis, mww=10000):
    ...
    # Handle empty sequence lists (reports with no procedures/steps)
    if len(ts_a) == 0 or len(ts_b) == 0:
        # Return format depends on min_dis/max_dis parameters
        if min_dis == 0. and max_dis == 0.:
            # Return tuple: (min_dtw_dis, max_dtw_dis)
            return 1.0, 1.0
        else:
            # Return single normalized distance value
            return 1.0
